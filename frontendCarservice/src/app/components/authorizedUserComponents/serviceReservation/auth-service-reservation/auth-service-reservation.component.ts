...
export class AuthServiceReservationComponent implements OnInit {
  ...
  dataServiceSetter() {
    ...
    for (const item of this.userCarList) {
  ...
  onSubmit(): void {
    ...
    if (!this.serviceReservationForm.valid) {
      ...
      this.serviceReservation.reserveAuthorizedServiceValidation(this.serviceReservationForm.getRawValue(),
        this.collector, this.selectedCar).subscribe(data => {
          this.dataService.user = data;
          for (let o of this.userCarList) {
            // @ts-ignore
            if (this.selectedCar == o.carId) {
              this.selectedCarObj = o;
            }
            this.dataService.selectedCarObj = this.selectedCarObj;
          }
